...
CONCLUSIONS: The findings highlight the superiority of systematic designs for producing smooth maps of optimal input levels and the impact of spatial variation on the performance of quadratic models in strip trials. Additionally, the study recommends fixed bandwidths based on experimental designs for GWR models. For a large strip trial to create a varying treatment map, a systemic design should be used as it has more flexibility in post-experiment statistical modelling.

IMPLICATIONS OR SIGNIFICANCE: The findings offer practical recommendations for designing large strip trials. By drawing attention to the considerations, especially regarding the experiment's purpose, this research contributes valuable insights for improving the efficacy and planning of future large strip trials.

% Context: While randomised designs are commonly preferred by agronomists and biometricians, the choice of design in on-farm trials often hinges on the experiment's objective. For instance, when creating a prescription map of continuous inputs across a grid in large strip trials, a systematic design may be more advantageous, although it typically receives less focus and discussion.

% Objective: This study evaluates the efficacy of systematic designs when combined with Geographically Weighted Regression (GWR) models in addressing spatial variation and estimating continuous treatment effects in large strip trials, using numeric simulations.

% Methods: We employed a Bayesian hierarchical model featuring spatially correlated random parameters to simulate data for various scenarios in large strip on-farm trials. The simulated data were then analysed using GWR models under two assumptions: linear and quadratic responses to treatment effects.

% Results: The results indicated that under a linear response assumption, the performance difference between systematic and randomised designs is negligible, regardless of spatial variation. However, for a quadratic response, the systematic design demonstrates superiority over randomised designs, evidenced by lower mean squared errors (MSE) on the model coefficients in GWR.

% Conclusions: Our findings underscore the effectiveness of systematic designs in generating smooth maps of optimal input levels and illustrate the influence of spatial variation on the performance of quadratic models in strip trials. The study also suggests adopting fixed bandwidths in GWR models based on the experimental design. For large strip trials aimed at creating a variable treatment map, a systematic design is recommended due to its greater flexibility in post-experimental statistical modelling.

% Implications: This research provides practical guidelines for the design of large strip trials, emphasising the importance of aligning the design with the experiment's objectives. It contributes valuable insights for enhancing the effectiveness and strategic planning of future large strip trials.
\end{abstract}
